...
Inherits iOSLibObject
	...
	#tag Method, Flags = &h0
		 Shared Function ReachToNodeWithDuration(TargetNode as ioslibsknode, RootNode As iosLibskNode, Duration As Double) As iOSLibSKAction
		  if iOS8Avaliable then
		    ...
		    result = new iOSLibSKAction (ReachToNodeWithDuration (classptr, TargetNode.id, RootNode.id, Duration))
		    result.RetainClassObject
		    return result
		  end if
	...
	#tag Method, Flags = &h0
		 Shared Function ReachToNodeWithVelocity(TargetNode as ioslibsknode, RootNode As iosLibskNode, Velocity As Double) As iOSLibSKAction
		  if iOS8Avaliable then
		    ...
		    #endif
		    result = new iOSLibSKAction (ReachToNodeWithVelocity (classptr, TargetNode.id, RootNode.id, velocity))
		    result.RetainClassObject
		    return result
		  end if
	...
	#tag Method, Flags = &h0
		 Shared Function ReachToPoint(aPoint as NSPoint, RootNode As iosLibskNode, Velocity As Double) As iOSLibSKAction
		  if iOS8Avaliable then
		    dim result as iOSLibSKAction
		    #if Target64Bit
		      ...
		      result = new iOSLibSKAction (reachToRNV (classptr, aPoint, RootNode.id, velocity))
		    #elseif Target32Bit
		      ...
		      result = new iOSLibSKAction (reachToRNV (classptr, aPoint.toNSPoint32, RootNode.id, velocity))
		    #endif
		    result.RetainClassObject
		    return result
		  end if
